...
/**
 * searching - searches for the value in an array of
...
int searching(int *array, size_t size, int value)
{
	...
	if (value < array[half])
		return (searching(array, half, value));

	half++;

	return (searching(array + half, size - half, value) + half);
}

/**
 * bn_search - calls to bn_search function to return
 ...
 * Return: index of the number
 */
int bn_search(int *array, size_t size, int value)
{
	int index;
...
int exponential_search(int *array, size_t size, int value)
{
	...
	result = bn_search(array + index, (next + 1) - index, value);
